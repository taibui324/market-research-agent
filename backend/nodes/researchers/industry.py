...
class IndustryAnalyzer(BaseResearcher):
    ...
    async def perplexity_search(self, query: str, max_results: int = 5) -> Dict[str, Any]:
        ...
        try:
            # Build user message with more specific instructions for industry data
            user_prompt = f"""Search and provide comprehensive industry analysis information about: {query}

Please include:
- Relevant URLs and sources
- Industry trends, market analysis, and competitive landscape
- Market size, growth rates, and industry challenges
- Key players, competitors, and market positioning
- Focus on industry insights, market dynamics, and business environment
...
Format your response with clear URLs and descriptions."""
            ...
            for i, url in enumerate(unique_urls[:max_results]):
                ...
                title = title_match.group(1).strip() if title_match else f"Industry Result {i+1}"
        ...
        try:
            ...
            if documents:  # Only process if we got results
                for url, doc in documents.items():
                    doc['query'] = doc.get('query', 'industry_analysis')
            ...
            if websocket_manager := state.get('websocket_manager'):
                if job_id := state.get('job_id'):
                    await websocket_manager.send_status_update(
                        job_id=job_id,
                        status="processing",
                        message=f"Used Perplexity Search to find {len(industry_data)} documents",
                        result={
                            "step": "Searching",
                            "analyst_type": "Industry Analyst",
                            "queries": queries
