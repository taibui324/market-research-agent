...
import { GlassStyle, AnimationStyle, CompetitorAnalyses } from '../types';
import CompetitorAnalysisComponent from './CompetitorAnalysis';

interface ResearchReportProps {
  output: {
    summary: string;
    details: {
      report_content: string; // Changed from 'report' to 'report_content'
...
const ResearchReport: React.FC<ResearchReportProps> = ({
  ...
  return (
    <>
      ...
      >
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          <div className="mt-4">
            ...
            >
              {output.details.report_content || "No report available"}
            </ReactMarkdown>
          </div>
        </div>
      </div>
      ...
      {console.log("ResearchReport checking competitor analysis data:", output?.details?.competitor_analyses)}
      {console.log("ResearchReport competitor analysis keys:", Object.keys(output?.details?.competitor_analyses || {}))}
      {console.log("ResearchReport competitor analysis length:", Object.keys(output?.details?.competitor_analyses || {}).length)}
      {output?.details?.competitor_analyses && Object.keys(output.details.competitor_analyses).length > 0 && (
