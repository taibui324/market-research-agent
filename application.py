...
async def process_research(job_id: str, data: ResearchRequest):
    ...
    try:
        ...
        report_content = report_data.get("report_content", "") or report_data.get("report", "")
        ...
        if not report_content:
            # Try to extract report from nested structure
            ...
            if not report_content:
                logger.error(f"No report content found in database for job {job_id}. Available keys: {list(report_data.keys()) if report_data else 'None'}")
...
async def process_enhanced_3c_analysis(job_id: str, data: MarketResearchRequest):
    ...
    try:
        ...
        # Extract report from final_state (generated by orchestrator)
        report_content = final_state.get('report', '')
        
        # If no report in final_state, try to get from database as fallback
        if not report_content:
            try:
                db_report = mongodb.get_report(job_id, analysis_type="3c_analysis")
                if db_report:
                    report_content = db_report.get('report', '')
                    logger.info(f"Retrieved report from database: {len(report_content)} characters")
            except Exception as e:
                logger.warning(f"Could not retrieve report from database: {e}")
                
        # Final fallback: try consumer_analysis_report for backward compatibility
        if not report_content:
            try:
                db_report = mongodb.get_report(job_id, analysis_type="consumer_analysis_report")
                if db_report:
                    report_content = db_report.get('report', '')
                    logger.info(f"Retrieved fallback report from database: {len(report_content)} characters")
            except Exception as e:
                logger.warning(f"Could not retrieve fallback report from database: {e}")
        
        logger.info(f"Final report content length: {len(report_content) if report_content else 0}")
